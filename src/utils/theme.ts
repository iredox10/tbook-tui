...
import { StyledText, type TextChunk, bg, fg } from "@opentui/core"
...
const plainChunk = (text: string): TextChunk => ({ __isChunk: true, text } as TextChunk)

/**
 * Format inline code as a styled TextChunk (theme-aware bg + fg).
 * Uses structured StyledText chunks instead of raw ANSI escapes, since
 * OpenTUI's text renderables do not parse inline ANSI in plain string content
 * (the escape digits would leak as visible text).
 */
export function formatInlineCode(code: string): TextChunk {
  const th = getTheme()
  return bg(th.inlineCode.bg)(fg(th.inlineCode.fg)(` ${code} `))
}

/**
 * Format inline rich text markers generated by the HTML parser.
 * Supports: `code`, **bold**, *italic*.
 *
 * Returns a StyledText so callers can compose it with other styled chunks.
 * Bold/italic markers are normalized to plain text (OpenTUI does not reliably
 * handle inline ANSI style toggles for bold/italic in plain string content,
 * which would leak as `1m`/`3m`). Inline code keeps terminal styling via a
 * proper bg/fg TextChunk.
 */
export function formatInlineRichText(text: string): StyledText {
  if (!text) return new StyledText([])
  ...
  while ((m = regex.exec(text)) !== null) {
    if (m.index > lastIdx) chunks.push(plainChunk(text.slice(lastIdx, m.index)))
    chunks.push(formatInlineCode(m[1]!))
    lastIdx = m.index + m[0].length
  }
  if (lastIdx < text.length) chunks.push(plainChunk(text.slice(lastIdx)))
