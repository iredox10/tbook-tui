 ...
 *
 * Note: OpenTUI text renderables do not reliably handle inline ANSI style
 * toggles for bold/italic in plain string content, which can leak as `1m/3m`.
 * So bold/italic markers are normalized to plain text, while inline code keeps
 * terminal styling.
 */
export function formatInlineRichText(text: string): string {
  ...
  // Inline code first so other replacements don't alter code markers.
  out = out.replace(/`([^`]+)`/g, (_m, code) => formatInlineCode(code))

  // Normalize markdown emphasis markers to plain text (no ANSI leakage).
